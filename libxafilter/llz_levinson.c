                  ...
                  float *acof, float *kcof, float *err)
{
    int   i, j;
    float acc;
    float tmp[LLZ_LEVINSON_ORDER_MAX+1];
    
    memset(tmp, 0, sizeof(float)*(LLZ_LEVINSON_ORDER_MAX+1));
    if (r[0] == 0.0) {
        for (i = 1; i <= p; i++) {
            acof[i] = 0.0;
            kcof[i] = 0.0;
                   ...
                   float *acof, float *kcof, float *err)
{
    int   i;
    int   k, s, m;
    float am1[LLZ_LEVINSON_ORDER_MAX+1];
    float km, em1, em;
    float errtmp;                    

    memset(am1, 0, sizeof(float)*(LLZ_LEVINSON_ORDER_MAX+1));
    if (r[0] == 0.0) {
        for (i = 1; i <= p; i++) {
            kcof[i] = 0.0;
            acof[i] = 0.0;
               ...
               float *x, float *kcof, float *err)
{
    int   i, j, k;
    float a, beta, q, c, h;
    float s[LLZ_LEVINSON_ORDER_MAX];
    float y[LLZ_LEVINSON_ORDER_MAX];

    memset(s, 0, sizeof(float)*LLZ_LEVINSON_ORDER_MAX);
    memset(y, 0, sizeof(float)*LLZ_LEVINSON_ORDER_MAX);
