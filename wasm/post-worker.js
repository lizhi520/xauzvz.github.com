...
var __xaudioprojs_running = false;

self.onmessage = function(e) {
  function makeOutHandler(cb) {
    var buf = [];
    return function(ch, exit) {
      if (exit && buf.length) return cb(__xaudioprojs_utf8ToStr(buf, 0));
      if (ch === 10 || ch === 13) {
        cb(__xaudioprojs_utf8ToStr(buf, 0));
        buf = [];
      } else if (ch !== 0) {
  ...
  var msg = e.data;
  if (msg["type"] == "run") {
    if (__xaudioprojs_running) {
      self.postMessage({"type": "error", "data": "already running"});
    } else {
      __xaudioprojs_running = true;
